...
func (attr *SchedAttr) SetTo(pid int, flags uint) error {
	...
	_, err := C.sched_setattr(C.pid_t(pid), &cAttr, C.uint(flags))
	return err
}

func (attr *SchedAttr) GetFrom(pid int, flags uint) error {
	cAttr := C.struct_sched_attr{}
	_, err := C.sched_getattr(C.pid_t(pid), &cAttr, C.uint(C.SCHED_ATTR_SIZE), C.uint(flags))
	if err == nil {
